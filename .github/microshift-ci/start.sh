#!/usr/bin/env bash
#
# Host-side helper to start a MicroShift container, wait until the API
# server is reachable and the node is Ready, then export a kubeconfig
# that targets it via 127.0.0.1.
#
# Usage:
...
# and the node never reaches Ready. Failing fast here saves ~10 min of
# pointless polling on misconfigured runs.
...
# point just burns the full WAIT_TIMEOUT. Check the unit state on every
# iteration and exit early on `failed`.
check_microshift_failed() {
    local state
    state=$(docker exec "$NAME" systemctl is-failed microshift.service 2>/dev/null || true)
    if [ "$state" = "failed" ]; then
        log "microshift.service entered 'failed' state — exiting early"
...
# server certificate is signed for in-cluster names, so TLS verification
# is disabled after rewriting the server to 127.0.0.1.
...
# dump the diagnostics that have surfaced real failures in the past:
#
#   - subscription-gated image pulls   -> visible in `describe pods`
#   - rshared mount-propagation error  -> visible in pod events
#   - OVS modprobe / OVN setup hang    -> visible in OVN pod logs,
#                                          OVS journals, and the
#                                          missing CNI config file
#   - cri-o EACCES exec'ing OVN CNI    -> visible in /run mount
#           binary (because /run is noexec)  options, ls of the binary,
#                                          and a direct --help exec
#
# Filtering on `phase != Running` misses the case where OVN pods are
# Running-but-not-functional, so we describe ALL pods (capped to
# head -200 to keep the workflow log readable).
dump_node_diagnostics() {
    ...
    # cri-o exec'ing the OVN CNI plugin returns EACCES if /run is
    # noexec. These dumps distinguish between (a) /run still mounted
    # noexec (entrypoint remount didn't take effect), (b) the binary
    # missing or not executable (OVN setup incomplete), and (c) AppArmor
