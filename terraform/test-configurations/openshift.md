...
No external DNS or ingress controller is needed — OpenShift Local configures a local DNS resolver so routes are accessible from the host at `*.apps-crc.testing`, and OpenShift has a built-in HAProxy-based router that handles routes natively. This wildcard applies to DNS name resolution only; see the certificate note in Step 4 for session hostname TLS limitations. OpenShift uses token-based auth instead of client certificates.
...
# Keycloak realm
curl -sk https://keycloak.apps-crc.testing/keycloak/realms/TheiaCloud/.well-known/openid-configuration
```
...
| Sessions     | `<session-uid>.ws.apps-crc.testing` |
| Keycloak     | `keycloak.apps-crc.testing`         |

> **Note on certificates:** session hostnames are **two** labels below the apps domain
> (`<session-uid>.ws.apps-crc.testing`). The default OpenShift/CRC wildcard certificate is
> `*.apps-crc.testing`, which only matches a single label and therefore does **not** cover
> session hostnames. Expect a browser certificate warning when opening a session on CRC.
> On a real cluster, provide a certificate covering `*.<instance-host>.<base-host>`
